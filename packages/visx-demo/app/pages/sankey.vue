    ...
    <template #controls>
      <div class="flex flex-wrap gap-4 text-sm">
        <label class="flex items-center gap-1">
          node alignment
          <select v-model="nodeAlignment" class="border border-gray-300 rounded px-1" @click.stop>
            <option v-for="key in Object.keys(nodeAlignments)" :key="key" :value="key">
              {{ key }}
            </option>
          </select>
        </label>

    ...
    <div ref="parentRef" class="chart-container bg-elevated/40 rounded-xl">
      <div v-if="width > 0">
        <div class="flex flex-wrap gap-3 mb-3 items-center">
          ...
          <USelect v-model="layout" :items="['cartesian', 'polar']" size="sm" />
          <template v-if="layout !== 'polar'">
            <span class="text-xs text-default font-medium">orientation:</span>
            <USelect v-model="orientation" :items="['horizontal', 'vertical']" size="sm" />
          </template>
          <span class="text-xs text-default font-medium">link:</span>
          <USelect v-model="linkType" :items="['diagonal', 'step', 'curve', 'line']" size="sm" />
        </div>
        <svg :width="width" :height="height">
          <LinearGradient id="links-gradient" from="#00DC82" to="#33e394" />

<template>
  <ExamplePage title="Gradients" :packages="['@visx-vue/gradient', '@visx-vue/shape']">
    <div ref="parentRef" class="chart-container">
      <svg v-if="width > 0" :width="width" :height="height">
        <template v-for="(gradient, index) in gradients" :key="`grad-${index}`">
          <component :is="gradient.component" v-bind="gradient.props" />
          <Bar

  ...
  >
    <div ref="parentRef" class="chart-container">
      <svg v-if="width > 0" :width="width" :height="height">
        <LinearGradient id="brush-gradient" from="#584153" to="#af8baf" :rotate="45" />
        <rect x="0" y="0" :width="width" :height="height" fill="url(#brush-gradient)" :rx="14" />

        <!-- Main chart -->
        <Group :left="margin.left" :top="margin.top">
          <LinearGradient id="area-grad" from="#af8baf" to="#af8baf" to-opacity="0.1" />
          <GridRows :scale="stockScale" :width="xMax" stroke="#e0e0e0" stroke-opacity="0.2" />
          <AreaClosed
            :data="filteredStock"
...
const bottomChartHeight = computed(
  () => innerHeight.value - topChartHeight.value - chartSeparation,
);

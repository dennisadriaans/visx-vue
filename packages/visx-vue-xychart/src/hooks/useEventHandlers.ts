...
/**
 * Composable that returns PointerEvent handlers that invoke the passed pointer
...
>({
  ...
  const { width, height, horizontal, dataRegistry, xScale, yScale } = inject(
    DataContextKey,
    {},
  ...
  function getHandlerParams(params?: HandlerParams): EventHandlerParams<Datum>[] {
    const { svgPoint, event } = params || {};
    const pointerParamsByKey: { [key: string]: EventHandlerParams<Datum> } = {};
  ...
  function handlePointerMove(params?: HandlerParams) {
    if (onPointerMove) {
      getHandlerParams(params).forEach((p) => onPointerMove(p));
    }
  }
  function handlePointerUp(params?: HandlerParams) {
    if (onPointerUp) {
      getHandlerParams(params).forEach((p) => onPointerUp(p));
    }
  }
  function handlePointerDown(params?: HandlerParams) {
    if (onPointerDown) {
      getHandlerParams(params).forEach((p) => onPointerDown(p));
    }
  }
  function handleFocus(params?: HandlerParams) {
    if (onFocus) {
      getHandlerParams(params).forEach((p) => onFocus(p));
    }
  }
  function handlePointerOut(params?: HandlerParams) {
    const event = params?.event;
    if (event && isPointerEvent(event) && onPointerOut) onPointerOut(event);
  }
  function handleBlur(params?: HandlerParams) {
    const event = params?.event;

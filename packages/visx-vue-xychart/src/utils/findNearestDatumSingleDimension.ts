...
import { bisector, range as d3Range, bisectLeft } from "@visx-vue/vendor/d3-array";
...
>({
  ...
  if ("invert" in coercedScale && typeof coercedScale.invert === "function") {
    ...
    nearestDatum =
      !nearestDatum0 ||
      Math.abs(dataValue - Number(accessor(nearestDatum0))) >
        Math.abs(dataValue - Number(accessor(nearestDatum1)))
        ? nearestDatum1
        : nearestDatum0;
